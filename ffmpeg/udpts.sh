...
ffmpeg -re -f concat -i list.txt -c copy -f mpegts -muxrate ${bitrate} -flush_packets 0 -y "udp://${output}?buffer_size=65536&pkt_size=1316&bitrate=${bitrate}&burst_bits=${burstbits}&fifo_size=120000"
rm list.txt
